...
| `WithCORS(origins...)`    | 开启 CORS(跨域)支持,传入允许的来源列表;传空则允许所有来源(`*`) | 不开启      |
...
默认不开启 CORS。若需要从浏览器页面或其他来源(如 [Scalar](https://client.scalar.com) 在线 API 调试工具)直接调用接口,需通过 `WithCORS()` 开启:

```go
s, err := httpserver.New(
	httpserver.WithAddr(":8080"),
	httpserver.WithCORS("https://client.scalar.com"), // 仅允许指定来源
	// httpserver.WithCORS(), // 或允许所有来源(*)
)
```

开启后服务会自动处理 `OPTIONS` 预检请求,并返回 `Access-Control-Allow-Origin` 等响应头。
